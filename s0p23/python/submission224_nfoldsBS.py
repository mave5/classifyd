...
model = '../caffe/net_deploy32.prototxt'
...
Xcrop = np.zeros([BS, 3, 224, 224], dtype=np.float32)  # initialize
...
for k1 in range(0,N,BS):
    print ('processing ', k1)
    if k1==79720:
        BS=6
    for k2 in range(BS):
	    im=Image.open(img_dir+imglist[k1+k2])
	    # downsample,subtract,crop data
	    Xcrop[k2,:,:,:]=preparedata(im)
    ...
    net1_outCnt[k1:k1+BS,:]=pred1[0:BS,:]
    net2_outCnt[k1:k1+BS,:]=pred2[0:BS,:]
    net3_outCnt[k1:k1+BS,:]=pred3[0:BS,:]
    net4_outCnt[k1:k1+BS,:]=pred4[0:BS,:]
    net5_outCnt[k1:k1+BS,:]=pred5[0:BS,:]
...
print("---execution time  %s mins ---" % exec_time)